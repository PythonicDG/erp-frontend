...
interface BulkUploadResult {
  ...
  errors: Array<{ row: number; standard_number: string; error_message: string }>;
  skipped: Array<{ row: number; standard_number: string; reason: string }>;
  successes: string[];
}

interface StandardBulkUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function StandardBulkUploadModal({ isOpen, onClose, onSuccess }: StandardBulkUploadModalProps) {
  ...
  const downloadTemplate = async () => {
    try {
      toast.loading('Generating template...', { id: 'download-std' });
      const blob = await standardsService.downloadTemplate();
      const url = window.URL.createObjectURL(new Blob([blob]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'standards_bulk_upload_template.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
      toast.success('Template downloaded successfully!', { id: 'download-std' });
    } catch (error) {
      toast.error('Failed to download template', { id: 'download-std' });
    }
  };
  ...
  const handleUpload = async () => {
    ...
    setLoading(true);
    const toastId = toast.loading('Uploading and processing standards...');
    try {
      const data = await standardsService.bulkUpload(file, skipDuplicates);
      setResult(data);
      
      if (data.success_count > 0) {
        toast.success(`Successfully imported ${data.success_count} standards!`, { id: toastId });
        onSuccess();
      } else if (data.failure_count > 0) {
        toast.error('All rows failed to import. See details below.', { id: toastId });
      } else {
        toast.success('File processed. No new standards created.', { id: toastId });
      }
    } catch (error: any) {
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Upload Excel sheet to quickly populate your standards master database.
            </p>
          </div>
          <button 
            onClick={handleClose} 
            ...
            <>
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <ul className="text-xs text-blue-700 mt-1.5 list-disc pl-4 space-y-1">
                    ...
                    <li>Required fields: <strong>Standard Number</strong>, <strong>Standard Name</strong>, <strong>Category</strong>.</li>
                    <li>Category must be one of: <code>ISO</code>, <code>IEC</code>, <code>Marine IEC</code>, <code>IP</code>, <code>EMC</code>, <code>Defence</code>.</li>
                    <li>Status field defaults to <code>Active</code> if empty or invalid.</li>
                    <li>Multiple standard entries can be uploaded in a single run.</li>
                  </ul>
                  <button 
              ...
              <div className="flex items-center gap-2 px-1">
                ...
                <label htmlFor="skip-duplicates" className="text-sm text-slate-700 font-medium cursor-pointer">
                  Skip duplicate records (based on same Standard Number)
                </label>
              </div>
            </>
          ) : (
                    ...
                    <div className="max-h-40 overflow-y-auto border-t border-slate-200 text-xs divide-y divide-slate-100 bg-white">
                      {result.skipped.map((skip, i) => (
                        <div key={i} className="px-4 py-2.5 flex items-start justify-between gap-4">
                          <div>
                            <span className="font-semibold text-slate-900">{skip.standard_number}</span>
